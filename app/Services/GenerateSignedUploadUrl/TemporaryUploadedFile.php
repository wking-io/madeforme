...
class TemporaryUploadedFile extends UploadedFile
{
    ...
    public function temporaryUrl()
    {
        ...
        if (FileUploadConfiguration::isUsingCloud() && ! app()->runningUnitTests()) {
        ...
        if (method_exists(Storage::getAdapter(), 'getTemporaryUrl')) {
            ...
            return Storage::temporaryUrl($this->path, now()->addDay());
        }

        return URL::temporarySignedRoute(
            'preview-file', now()->addMinutes(30)->endOfHour(), ['filename' => $this->getFilename()]
        );
    }
